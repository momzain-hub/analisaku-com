  ...
  function init(anchor){
    ...
    function renderTable(){
      ...
      body.innerHTML=view.map(r=>{
        if(!r.data)return `<tr class="signal-row unavailable"><td><b>${r.symbol}</b></td><td>—</td><td>NO DATA</td><td>—</td><td>—</td><td>—</td><td>—</td><td>—</td></tr>`;
        const d=r.data,s=String(d.status||'WAIT').toUpperCase(),trend=String(d.trend||'NEUTRAL').toUpperCase();
        return `<tr class="signal-row" data-symbol="${r.symbol}"><td><b>${r.symbol}</b></td><td><span class="signal-trend ${trend.toLowerCase()}">${trend}</span></td><td><span class="signal-pill ${statusClass(s)}">${s}</span></td><td>${fmt(d.trigger)}</td><td>${entry(d)}</td><td>${fmt(d.invalidation)}</td><td>${fmt(d.target1)}</td><td>${updated(d)}</td></tr>`;
    ...
    function openSymbol(symbol){
      const ticker=document.getElementById('tvTicker'),interval=document.getElementById('tvInterval'),apply=document.getElementById('tvApply');
      if(ticker)ticker.value=symbol;if(interval)interval.value='D';if(apply)apply.click();
    ...
    async function ensureApi(){
      ...
      await new Promise(resolve=>{const s=document.createElement('script');s.src=new URL('signal-config.js',moduleSrc).href;s.onload=resolve;s.onerror=resolve;document.head.appendChild(s);});
      api=String(window.ANALISAKU_SIGNAL_API||'');
    }
    function technicalEndpoint(){
      const url=new URL(api);url.pathname=url.pathname.replace(/\/signal\/?$/,'/technical');url.search='';url.searchParams.set('timeframe',tf);return url;
    }
    async function loadGoldenCross(){
      await ensureApi();if(!api){$('gcMeta').textContent='Signal API belum tersedia.';return;}
      try{
        const res=await fetch(technicalEndpoint(),{cache:'no-store'});if(!res.ok)throw new Error();gcPayload=await res.json();
        const s=gcPayload.summary||{},stamp=Number(gcPayload.updated_at)||0;
      ...
      }catch(e){$('gcMeta').textContent='Golden Cross Radar belum dapat dimuat. Coba Refresh.';}
    }
    async function load(){
      await ensureApi();if(!api){$('signalMonitorMeta').textContent='Signal API belum tersedia.';return;}
      const results=await Promise.all(symbols.map(async symbol=>{try{const url=new URL(api);url.searchParams.set('ticker',symbol);url.searchParams.set('timeframe',tf);const res=await fetch(url,{cache:'no-store'});if(!res.ok)return {symbol,data:null};const body=await res.json();return {symbol,data:body.data||body};}catch(e){return {symbol,data:null};}}));
      rows=results;const available=rows.filter(r=>r.data).length,latest=Math.max(0,...rows.map(r=>ts(r.data)));
      $('signalMonitorMeta').textContent=`${available}/20 snapshot tersedia`+(latest?` • update terakhir ${new Date(latest).toLocaleString('id-ID')}`:'');renderSummary();renderFilters();renderTable();
    }

    $('gcRefresh').addEventListener('click',loadGoldenCross);$('signalMonitorRefresh').addEventListener('click',load);$('signalBullishToggle').addEventListener('click',()=>{bullishOnly=!bullishOnly;renderFilters();renderTable();});
    renderFilters();renderGcFilters();loadGoldenCross();load();setInterval(()=>{loadGoldenCross();load();},60000);
  }
  boot();
})();
