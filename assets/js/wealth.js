/* Analisaku Wealth Management v1.9.4 bootstrap */
(function(){
  ...
  async function init(){
    ...
    loadCss(url('../css/wealth-v19.css?v=1.9.4-20260916-0930'),'wealthV19');
    ...
    try{
      ...
      await loadScript(url('wealth-pdf-feedback.js?v=1.9.3-20260916-0119'));
      window.ANALISAKU_WEALTH_MODE_API?.apply?.();
      setTimeout(()=>{
        window.ANALISAKU_WEALTH_ANSWER_STATE?.restore?.();
        window.ANALISAKU_WEALTH_COPY?.apply?.();
        window.ANALISAKU_WEALTH_PDF?.patch?.();
        const badge=document.querySelector('.wm-version-badge');
        if(badge)badge.textContent='WEALTH v1.9.4';
      },80);
    }catch(error){console.error('Wealth Management v1.9.4 gagal dimuat',error);}
  }
